...
|---|---|---|
| INV-1 | One event per commit: native_id is the SHA, kind is commit.recorded, source is git | sync_test.go::TestSync_MintsOneCommitRecordedEventPerCommit |
| INV-2 | A second Sync over unchanged history appends ZERO | sync_test.go::TestSync_SecondSyncAppendsNothing |
| INV-3 | Appended + Existing == Listed, and Appended counts only inserts — asserted in BOTH directions | sync_test.go::TestSync_ResultCountsPartitionTheListingExactly |
| INV-4 | The payload is the Commit marshalled canonically; a changed field changes content_sha | payload_test.go::TestPayload_FieldChangeChangesTheContentSHA |
| INV-5 | The same commit read twice yields the same content_sha; slices are sorted and de-duplicated | payload_test.go::TestPayload_IsStableAcrossReads |
...
| INV-10 | A shuffled commit list yields the same events and the same counts | sync_test.go::TestSync_ListingOrderDoesNotChangeTheOutcome |
| INV-11 | A failed append aborts the Sync, reports the cause, and states partial progress | sync_test.go::TestSync_AppendFailureAbortsAndReportsProgress |
| INV-12 | A malformed commit surfaces core's validation error rather than being sanitised | sync_test.go::TestSync_MalformedCommitIsRefusedByCore |
| INV-13 | The pinned payload vector marshals to the exact bytes and content_sha in § 5.1 | payload_test.go::TestPayload_PinnedVector |
| INV-14 | § 2's declared surface is exactly what the package exports | speclock_test.go::TestExportedSurfaceMatchesTheSpec |
...
| INV-18 | A commit dated before one already ingested is still ingested | contract_test.go::TestSync_ACommitOlderThanAnyAlreadySeenIsIngested |
| INV-23 | The cursor is the repository's real tips | contract_test.go::TestSync_CursorIsTheRepositoryTips |
| INV-26 | Version's value is pinned | contract_test.go::TestVersion_IsPinned |
| INV-27 | Every dependency is required at construction | contract_test.go::TestNew_RequiresEveryDependency |
| INV-4b | An empty slice normalises to null, pinned by a second vector | payload_test.go::TestPayload_EmptySlicesArePinnedToNull |
| INV-14b | § 2's struct FIELDS are the complete exported field set | speclock_test.go::TestExportedStructFieldsMatchTheSpec |
