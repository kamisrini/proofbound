# Proofbound — Build Constitution

Proofbound is the trust layer for AI-built software: an event-sourced delivery record (Flight Recorder), intent fabric, living twin, and trust engine. Vision: [vision-2028.md](vision-2028.md). Roadmap: [ROADMAP.md](ROADMAP.md).
...
**This repo is built under Proofbound's own laws.** If the way we build contradicts what we're building, the build is wrong.
...
6. **Spec before code.** Every kernel package gets a `SPEC.md` (interface, invariants, non-goals, test derivations) before implementation — `/proofbound-spec`. Tests derive from the spec.
...
Files + git + grep ARE the record during bootstrap. Derived indexes only (`make index`). The kernel itself becomes the project's record once it exists — **Proofbound's first tenant is Proofbound** (self-hosting: ingest this repo's commits, check-runs, and agent sessions as the first connector).

## Session protocol

- **Start:** the SessionStart hook prints `notes/state.md` + git status. Run `/proofbound-next` to orient and pick the next task.
- **End:** use the single canonical EOD prompt in [docs/eod-prompt.md](docs/eod-prompt.md). It owns
  ...
  `/proofbound-wrap` is optional only when that command exists in the checkout.
- **Decisions during work:** `/proofbound-decide` immediately, not at session end.
- **Before merging a meaningful chunk:** `/proofbound-review` (adversarial pass against these laws + the spec).
- **Parallel sessions:** exactly one session owns `notes/` writes and `/proofbound-wrap` at a time; subagents never write `notes/state.md` or the journal — they return results to the owning session; parallel worktree sessions write journal fragments to `notes/tmp/` and the owner merges at wrap.
