...
not run the repository gate because native `make` is absent. The next work is to commit those
integrity artifacts, prove the frozen commit from a fresh detached Linux clone, then obtain the
founder's platform-scope decision required by the Task 3 contract.
