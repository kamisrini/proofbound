...
P6 Task 5 is implemented spec-first. The new checker and closure packet enumerate the two
...
**Exact next action:** commit the regenerated Task 2 evidence refresh together with this state and
journal update, then run host `PATH=/home/thamm/go/bin:$PATH make check` and the Task 5 acceptance
commands. Treat the sandbox Snap capability error as environmental; any other failure is real. If
all gates are green, update the state to begin Task 6 (measurements and falsifier evaluation). Do
not widen P6 scope.
