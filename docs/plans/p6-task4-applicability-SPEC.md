...
At the existing controlled boundary, `scripts/intent-delivery-applicability.sh` rejects `HEAD` when
the path predicate matches and the commit has no explicit `Intent:` trailer. It does not synthesize
or resolve a claim; the existing post-sync intent-reference/readiness gates continue to reject
malformed, dangling, withdrawn, or otherwise non-verifiable exact references. The seeded boundary
check is `scripts/tests/intent-delivery-applicability.test.sh`.
