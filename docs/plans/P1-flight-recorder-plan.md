...
| 5 | witness emitter + checks connector | ◐ ROUND-1 REMEDIATED — spec-first implementation, real shell emission, DB-backed `vera sync checks` replay, connector 37/37 and CLI 17/17 killed; Round 1 `NEEDS_WORK` is committed and a new independent verdict is required. The prescribed `.claude/commands/vera-wrap.md` does not exist in this checkout, so its step 3 cannot be amended here and remains explicit migration debt |
